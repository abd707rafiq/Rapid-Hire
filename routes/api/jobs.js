...
router.post('/',companyAuth, async (req, res) => {
  ...
  try {
    if (!req.files || !req.files.jobs) {
      return res.status(400).send('No file uploaded');
    }

    const { name, data } = req.files.jobs;
    ...
    const jobPath = filePath; // Store the path to the uploaded resume
...
router.post('/apply/:id', auth, async (req, res) => {
  try {
    ...
    const applicant = {
      ...
      field: profile.field,
      avatar: user.avatar,
      resume: resumePath // Store resume path in applicant object
    }
